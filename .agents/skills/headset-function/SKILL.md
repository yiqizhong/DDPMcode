...
description: Build a headset sub-page function CARD (a setting/feature like Noise Control, Collaboration, Multimedia) from the card shell + component snippets, filling manifest values. Use for any sub-page function that has no dedicated snapshot yet — never hand-roll function markup from a description.
...
headset-shared/components/<archetype>.html           ← the components (toggle / slider / …)
headset-shared/components/info-tooltip.html          ← the optional ⓘ + tooltip
```

A card = **shell + a list of components**. Both come from files — never write the markup from the
...
- `$1` — the function id. Plus that function's `functions[]` entry from the sub-page manifest: its
  `title`, optional info text, and its **components** (each: an archetype + that archetype's value
...
2. **Title ⓘ** (`data-slot="function-info"`): if the function has info text, COPY
   `headset-shared/components/info-tooltip.html` into it and fill `{info-text}`; else delete that div.
3. **Components** (`data-slot="components"`): for each component, in order, COPY the matching
   `headset-shared/components/<archetype>.html` into `.function-content` and fill its value slots
   ...
   per §9.4 (then it can be promoted to `components/<archetype>.html`).
   **Segmented icons:** when a `segmented` component has `icons: true`, copy each option's icon from
...
4. **Conditional reveals** (`reveals` on a selector component — segmented | preset-grid): keep the
   ...
   list for option N's `value`: a component slot → COPY `components/<archetype>.html` (recurse:
   ...
   A component must NOT carry a flat `condition:` field — that is the pre-schema form; its content
...
5. **Toggle dependents (greying, not reveal)** — driven by a `toggle`'s `dependents` field: its
   ...
   EACH dependent in its own `.subfn-child`. Copy each dependent's `components/<archetype>.html` (or
   `{ function: <id> }`, recursive) into its `.subfn-child`. If a dependent is a full-width component
...
- **COPY the shell + COPY each component snippet — never generate the markup from the function or
...
- Each component COPIED from `headset-shared/components/<archetype>.html`, values from the manifest?
