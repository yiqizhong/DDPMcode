...
| D30 | **Sole-toggle master/member fork made explicit + restatement gate** — a card whose sole top-level component is a `toggle` has two *equally legal* shapes, disambiguated only by the toggle's `label`: **single-feature card → master switch** (NO label; hoisted to the title row per D28) vs **group card → named member** (a label distinct from the title, e.g. `Collaboration`→`Mic Noise Canceling`). Both stay legal (user decision 2026-07-02: allow both). The defect was that the redundancy guard added in D24a only caught `label == title` **exact** equality, so a near-synonym restatement (`Volume Tone` under `Volume Adjustment Tone`) passed validation yet silently rendered as an orphan body row instead of a master switch. Fix (this-round scope = **toggle only**): `validate-manifest.py` widens the check for a sole top-level toggle from equality to **word-subset restatement** (`restates_title`: label word-set ⊆ title word-set; one-directional — a title-expanding label like `Enable Dell Audio Promotion` under `Dell Audio Promotion` is the legit "Enable <feature>" idiom and stays legal), and `headset/AGENTS.md` documents the fork explicitly. No new schema field, no renderer change, no manifest edits (WL1227 `volume-adjustment-tone` intentionally left failing per user; WL1127 was already correct = label-less). | Codex gpt-5.5 independent review of the proposed root-cause + fix: my first proposal (blanket "sole toggle must not have a label") would have false-flagged the legit `Collaboration` group cards (FIXTURE / WL327); the real root cause is `label`-presence *overloaded* across two legitimate shapes, disambiguated by semantic (dis)similarity that an exact-match guard can't see. | The structural bit ("master switch or named member?") was carried implicitly by an optional cosmetic field and guarded only by a heuristic too narrow to catch synonymy; the same NL requirement authored correctly for `Audio Guidance` / WL1127 but wrongly for WL1227 proves it was free-judgment variance, not a comprehension error. Deferred: the twin `sole full-width label silently dropped` case (Multimedia `Presets`, render-content.py:343) — same class, separate round to avoid cross-model HTML regen. | Superseded in part by D31: the fork + `restates_title` detector stay, but its enforcement is downgraded from a HALT to a non-blocking advisory (a word-subset is only a proxy for a semantic call). |
| D31 | **Restatement is an advisory, not a gate + tier-2 authoring recheck** — the D30 word-subset HALT was a semantic judgment ("is this label a redundant re-wording of the title?") forced into string set-math; it necessarily leaks both ways (misses no-overlap synonyms like `Beep On Change`; would false-flag a legit member whose name sub-sets the title) and the fact that D30 had to be narrowed to one-directional just to dodge the legit `Enable <feature>` idiom is itself the tell that it over-reached the mechanical-gate philosophy. New design (all decisions user 2026-07-02): (1) **detection stays** but `restates_title` is downgraded from `err` → **non-blocking `advisory`** in `validate-manifest.py`; exact `label == title` stays a hard error (D24a, a true duplicate). (2) `verify-all.sh` **harvests advisories and prints one consolidated REVIEW FLAGS summary at the end, exit 0** — a *pure reminder*, never blocks the build/commit. (3) The real resolution moves to **authoring**: when the manifest-author (an LLM) hits a restatement, it re-reads the requirement and decides master/member **once per field** (read B, no loop); still-unsure cases are left and flagged, never hard-stopped. The LLM recheck lives in the **authoring phase** (it edits the manifest before finalize), so `render-model.py` stays fully deterministic — no LLM in the renderer. | User pushback: the ⊆ rule is "rule-correct but semantically off"; hard mechanical rules are the wrong tool for a semantic/taste call. Extends the D29 AUTOFIX/ESCALATE taxonomy with the missing middle tier — *semantic-but-requirement-resolvable* → route to the requirement-reader (LLM at authoring, human as backstop) — instead of miscategorizing it as machine-determinable. | A mechanical gate is right for true invariants (counts, enums, ranges, exact-dup) but wrong for a semantic distinction; the honest move is to let the detector only *raise suspicion* (recall-oriented, false-positives OK) and hand the *judgment* to whoever holds the requirement, while keeping the deterministic renderer LLM-free. Deferred (unchanged from D30): the twin `sole full-width label silently dropped` case (Multimedia `Presets`). Optional future: §7-style "infer once → encode a reusable rule → stop calling the LLM" on top of read-B. | Active: advisory downgrade + REVIEW FLAGS summary shipped; full suite green (test-regen back to green — WL1227 no longer HALTs, renders + is flagged); exact `label==title` still HALTs; AGENTS.md documents the tier-2 authoring recheck. The tier-2 closed loop (path b) is now IMPLEMENTED as `author-recheck.py`: at authoring it validates → for each restatement flag consults an LLM with the requirement (once per field, read B; pluggable via `AUTHOR_RECHECK_LLM_CMD`/`AUTHOR_RECHECK_MODEL`, stubbable via `AUTHOR_RECHECK_DECISIONS`) → MASTER deterministically drops the toggle's `label:` line and re-validates, MEMBER/undecided stays flagged. Edits the manifest before render, so `render-model.py` stays LLM-free/deterministic. Verified: `test-author-recheck.sh` (stub, 4 cases) + a live gpt-5.4-mini run that decided MASTER and cleared the flag on a scratch copy (real WL1227 untouched). |
