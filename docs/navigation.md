...
| **改"可替换控件(swap)"规则** | **只改** `subcontrols/control-row.html`(唯一权威);`single-control.html` 引用它,别在它里重写 | 只允许 switch ↔ dropdown 进 header |
...
| swap 规则(可替换 header 控件) | `subcontrols/control-row.html` | single-control 引用它 |
