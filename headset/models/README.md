...
2. Write the manifests (清单) — content only, no HTML:
   - `home.manifest` — identity (marketing name, model number, firmware, PPID), device image,
     ...
     `link`).
   - one `<subpage>.manifest` per feature link — a `title` + a `functions[]` list (each function
     = `id` + parameters). `functions[]` may be empty.
...
3. Generate the pages **through the skills** (see `../AGENTS.md`), never by hand:
   ...
   - functions → copy `templates/functions/<id>.html` if it exists, else `@skills:headset-function`
